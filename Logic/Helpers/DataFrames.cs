...
namespace binanceBotNetCore.Logic.Helpers
{
    public static class DataFrames
    {
        ...
        public static DataFrame CountShouldBuy(DataFrame df)
        {
            for (int index = 0; index < df.Rows.Count; index++)
            {
                df[index, 69] = false;
                if (index < df.Rows.Count - 15)
                {
                    decimal close = Convert.ToDecimal(df[index, 5]);
                    close = close + (close / 100m);
                    for (int local_index = 0; local_index < 15; local_index++)
                    {
                        if (close < Convert.ToDecimal(df[index + local_index, 3]))
                        {
                            df[index, 69] = true;
                        }
                    }
